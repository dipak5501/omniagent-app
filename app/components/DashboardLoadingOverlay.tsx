...
function useProgress() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setValue((v) => {
        if (v >= 99) {
          return 30; // loop subtly so it looks alive
        }
        // accelerate at the start, slow near the end
        const delta = v < 50 ? 2.5 : v < 80 ? 1.5 : 0.6;
...
const dashboardTasks = [
  {
    id: 'init',
    label: 'Initializing dashboard interface',
    icon: LayoutDashboard,
  },
  { id: 'data', label: 'Loading treasury data', icon: BarChart3 },
  { id: 'users', label: 'Syncing user permissions', icon: Users },
  { id: 'config', label: 'Configuring AI agent', icon: Settings },
  { id: 'ready', label: 'Preparing governance tools', icon: Activity },
];
  ...
  const activeIndex = useMemo(
    () =>
      Math.min(
        ...
        Math.floor((progress / 100) * dashboardTasks.length)
      ),
    [progress]
  );
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/90 backdrop-blur-md isolate">
      <div className="relative w-full max-w-2xl mx-auto px-6">
        ...
        <div className="relative mb-8 grid place-items-center">
          ...
          >
            <div
              ...
              style={{
                background:
                  'conic-gradient(from 0deg, rgba(139,92,246,.25), transparent 40%, rgba(59,130,246,.25), transparent 75%, rgba(236,72,153,.25))',
                maskImage:
            ...
            {[0, 1, 2].map((i) => (
              <Motion.span
                key={i}
                className="absolute left-1/2 top-1/2 h-2 w-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-violet-300 shadow-[0_0_16px_rgba(139,92,246,0.8)]"
          ...
          <div className="absolute h-20 w-20 rounded-2xl border border-white/15 bg-gradient-to-b from-white/15 to-white/5 backdrop-blur">
            ...
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
              <LayoutDashboard className="h-10 w-10 text-violet-300" />
            </div>
            {/* Scanning effect */}
            <Motion.div
              className="absolute left-1/2 top-[52px] h-1 w-12 -translate-x-1/2 rounded bg-gradient-to-r from-violet-400 via-blue-300 to-fuchsia-300"
        ...
        <p className="max-w-xl text-center text-white/80 text-sm mb-6">
          Preparing your DAO governance interface and AI agent dashboard.
          Loading treasury data and configuring your workspace.
        </p>
        ...
        <ul className="grid w-full grid-cols-1 gap-2 md:grid-cols-2">
          {dashboardTasks.map((t, idx) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`h-4 w-4 ${active ? 'text-violet-300' : 'text-white/70'}`}
                />
                <span
              ...
              >
                {Array.from({ length: 12 }, (__, row) => {
                  ...
                  return (
                    <Motion.span
                      key={rowId}
                      className="absolute left-0 right-0 h-1 bg-gradient-to-r from-transparent via-violet-400/60 to-transparent"
                      initial={{ y: -8 }}
                      animate={{ y: [-8, 80] }}
                      transition={{
                        duration: 1.6 + (row % 5) * 0.3,
                        repeat: Infinity,
                        delay: (row % 7) * 0.1,
                      }}
                      style={{ top: `${row * 8}px` }}
